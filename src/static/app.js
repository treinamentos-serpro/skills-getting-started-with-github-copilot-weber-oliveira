document.addEventListener("DOMContentLoaded", () => {
            ...
            ${
              details.participants.length
                ? `<ul>${details.participants
                    .map(
                      (participant) =>
                        `<li><span>${participant}</span><button type="button" class="delete-participant" data-activity="${encodeURIComponent(name)}" data-participant="${encodeURIComponent(participant)}" aria-label="Remover ${participant}" title="Remover participante">&#128465;</button></li>`
                    )
                    .join("")}</ul>`
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    const email = decodeURIComponent(deleteButton.dataset.participant);
